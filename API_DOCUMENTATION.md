...
const searchPapers = (params) => api.get('/api/papers/search', { params });
const getPaper = (id) => api.get(`/api/papers/${id}`);
const getTrendingKeywords = () => api.get('/api/trends/keywords');
const getKeywordTrend = (keyword) => api.get(`/api/trends/keyword/${keyword}`);
const compareTrends = (keywords) => api.get('/api/trends/compare', { params: { keywords } });
const getDashboard = () => api.get('/api/dashboard/public');
const getUserDashboard = () => api.get('/api/dashboard/me');
const getBookmarks = (params) => api.get('/api/bookmarks', { params });
const addBookmark = (data) => api.post('/api/bookmarks', data);
const removeBookmark = (id) => api.delete(`/api/bookmarks/${id}`);
const getNotifications = () => api.get('/api/notifications');
const markRead = (id) => api.patch(`/api/notifications/${id}/read`);
const getFollows = () => api.get('/api/follows');
const follow = (data) => api.post('/api/follows', data);
const unfollow = (id) => api.delete(`/api/follows/${id}`);
```
